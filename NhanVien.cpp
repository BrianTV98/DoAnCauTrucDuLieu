...
void InsertNhanVien(LISTNHANVIEN &ls,NhanVien nv){
	...
	if(ls.sl==-1){
		...
		ls.dsnv[ls.sl]->DSHoaDon=nv.DSHoaDon;
	}
	else{
		if(Search_infor_NhanVien(ls,nv.MANV)) return; // fix bug ... ma trung van load tu file len
...
B:	if(FullNhanVien(ls)==true){
		...
		KhungTrangThai(HCN(3,49),HCN(40,50),1);
...
//				if(searchNhanVienBinary(ls,nv.MANV,0,ls.sl)){
				if(Search_infor_NhanVien(ls,nv.MANV)){
					...
					strcpy(nv.HO,getHoNhanVien());
					gotoxy(23,wherey()+1);
					nv.PHAI=(getLoai('m','f')=='M')?true:false;
					nv.DSHoaDon=NULL;
...
int searchIndexInsertBinary(ListNhanVien ls, char *Ma, int low, int high) { 	
	//tra ve vi tri neu tim thay nhan vien, 
	// tra ve vi tri hop ly co the chen neu  ko tim thay nhan vien
	if(ls.sl==-1) return -1;
//	if(strcmp(ls.dsnv[0]->MANV,Ma)==0) return 0; // fix bug: ko cho chen vao dau danh sach
    if (high <= low)  {
    ...
    if(strcmp(Ma,ls.dsnv[mid]->MANV)==0){
  	...
  	if(strcmp(Ma,ls.dsnv[mid]->MANV)>0){
  		return searchIndexInsertBinary(ls, Ma, mid+1, high); 
	  }
	  
	return searchIndexInsertBinary(ls, Ma, low, mid-1); 
} 
...
//	string tenDau=string(ls.dsnv[0]->TEN)+string(ls.dsnv[0]->HO); // fix bug ko tra ve vi tri 0 duoc
//	if(strcmp(tenDau.c_str(),tenHo.c_str())<0) return 0; // fix bug ko tra ve vi tri 0;
...
editNhanVien:	KhungTrangThai(HCN(3,16),HCN(40,17),0);
	...
	if(is_EmptyNhanVien(ls)) TrangThai(-1,-1,"D/s Vat Tu Rong!");
		...
		p=Search_infor_NhanVien(ls, buffer);
		if(p){
			showNhanVienDon(*p);
			vitri= Search_index_NhanVien(ls,buffer);
			...
			NhanVien nv;
			gotoxy(23,16);	
NhapLaiMa:	//strcpy(nv.MANV,getMaVatTu()); // nhap vao ma nhan vien moi // fix bug-> tat tinh nang sua ma
//			if(strcmp(nv.MANV,"0")!=0){	
//				if(Search_infor_NhanVien(ls,nv.MANV)){ //
//					TrangThai(-1,-1,"Ma Trung! Nhap Lai!");
//					goto NhapLaiMa;
...
//		NODEVATTU rp,p;
		...
		colorDialogue();
		nohidecursor();
		gotoxy(4,16);cout<<"Ma Nhan Vien Can Xoa: ";
		strcpy(buffer, getMaVatTu());
...
//			int vitri=searchIndexInsert(ls,buffer,0,ls.sl);
			int vitri=Search_index_NhanVien(ls,buffer);
			cout<<"Ban Co muon xoa Nhan Vien nay Khong. Thao Tac Xoa se ko the hoan tac!(y/n)";
