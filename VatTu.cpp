...
void Create_nodeVatTu(NODEVATTU &root){
	...
	do{
		...
		if(strcmp(vt.MAVT,"0")!=0){
			if(search(root,vt.MAVT) ){ // kiem tra ma trung! // fix bug tu dong cho nhap
				TrangThai(-1,-1,"Ma Trung!");
				if(TiepTuc()==true){
					continue;
				}
				else break;
...
bool HieuChinhSoLuongTon(NODEVATTU root,char *ma,int n){
	...
	string str="Ton Kho: "+ to_string(p->vatTu.SLT);
	TrangThai(-1,-1,str);
	return false;
}
...
A:	KhungTrangThai(HCN(3,16),HCN(40,17),0);
	...
	if(is_Emty(root)) TrangThai(-1,-1,"D/s Vat Tu Rong!");
		...
		if(p){
			...
			gotoxy(4, wherey()+2);
			KhungTrangThai(HCN(2,wherey()),HCN(100,wherey()+1),0);
			gotoxy(3,wherey());cout<<"Ban Co muon xoa Vat Tu Khong. Thao Tac Xoa se ko the hoan tac!(y/n):";
...
hieuchinh:
	...
	if(is_Emty(root)) TrangThai(-1,-1,"D/s Vat Tu Rong!");
		...
		if(p){
			...
			showVatTuDon(p->vatTu);
			VatTu vt;
			strcpy(vt.MAVT,p->vatTu.MAVT); // fix bug ... ko cho sua ma vat tu nua
			vt.SLT=p->vatTu.SLT;
			remove(root,buffer);
